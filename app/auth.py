...
def activate():
    try:
        ...
        if request.method == ('GET'): 
            ...
            query = "SELECT *FROM activationlink WHERE challenge ="+number+"AND state ="+utils.U_UNCONFIRMED
            attempt = db.execute('SELECT *FROM activationlink WHERE challenge = ? AND state = ?', (number, utils.U_UNCONFIRMED)).fetchone()

            if len(attempt) > 0:

                query = "UPDATE activationlink SET state = "+"'"+utils.U_CONFIRMED+"'"+" WHERE id ="+str(attempt['id'])
                print(query)
                db.execute(query)
                db.commit()
...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            consulta = db.execute(query).fetchone()
            ...
            query = "SELECT user,password FROM credentials WHERE name ="+"'"+utils.EMAIL_APP+"'"


            print(query)
            sentencia = db.execute(query)
            resultado = sentencia.fetchone()

            content = 'Hello there, to activate your account, please click on this link ' + url_for('auth.activate', _external=True) + '?auth=' + number
            
            send_email(resultado, receiver=email, subject='Activate your account', message=content)
